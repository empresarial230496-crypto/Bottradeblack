...
// ── Config ────────────────────────────────────────────────
const API_KEY    = process.env.API_KEY    || process.env.BYBIT_API_KEY    || '';
const API_SECRET = process.env.API_SECRET || process.env.BYBIT_API_SECRET || '';
...
function bybitHeaders(payload) {
  ...
  // ✅ ORDEN CORRECTO: timestamp + apiKey + recvWindow + body
  ...
  return {
    ...
    'X-BAPI-SIGN':         signature,   // ✅ CORREGIDO (era X-BAPI-SIGN-HEADER)
  };
}
...
// ── Estado del bot ────────────────────────────────────────
const botState = {
  running: false, config: null, currentPosition: null,
...
app.get('/health',  (_, res) => res.json({ ok: true, uptime: process.uptime(), testnet: TESTNET, running: botState.running }));
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (result.retCode === 0) {
      log('ok', `✅ Orden ${data.action} ${data.symbol} ejecutada — ID: ${result.result?.orderId}`);
      res.json({ ok: true, orderId: result.result?.orderId });
    } else {
      log('err', `Bybit error: ${result.retMsg}`);
      res.json({ ok: false, error: result.retMsg });
    }
  } catch (e) {
    log('err', 'Error orden: ' + e.message);
...
app.post('/api/start', (req, res) => {
  const cfg = req.body;
  if (!API_KEY || !API_SECRET) return res.json({ ok: false, error: 'Faltan API keys en .env' });
  if (!cfg.pairs?.length)       return res.json({ ok: false, error: 'Selecciona un par' });
...
async function getIndicators(pair) {
  // ✅ GET request — firma va en query string, NO en body
  const timestamp  = Date.now().toString();
  const recvWindow = '5000';
...
async function executeTrade(side, symbol, price) {
  ...
  try {
    ...
    await placeOrder({ category: 'linear', symbol, buyLeverage: String(cfg.leverage), sellLeverage: String(cfg.leverage) });
  } catch(_) { /* ignora si ya está seteado */ }
  ...
  if (result.retCode !== 0) { log('err', `Orden error: ${result.retMsg}`); return; }
  ...
  const monitor = setInterval(async () => {
    ...
    try {
      ...
      const r = await axios.get(`${BASE_URL}/v5/position/list?${queryStr}`, {
        headers: { 'X-BAPI-API-KEY': API_KEY, 'X-BAPI-TIMESTAMP': timestamp, 'X-BAPI-RECV-WINDOW': recvWindow, 'X-BAPI-SIGN': signature }
      });
...
async function closePosition(pos) {
  try {
    await placeOrder({ category: 'linear', symbol: pos.symbol, side: pos.side === 'Buy' ? 'Sell' : 'Buy', orderType: 'Market', qty: String(pos.qty), reduceOnly: true });
