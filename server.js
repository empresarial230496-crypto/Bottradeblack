...
function bybitHeaders(payload) {
  ...
  return {
    ...
    'X-BAPI-SIGN':        signature,   // ✅ nombre correcto
  };
}
...
app.post('/webhook', async (req, res) => {
  ...
  // Validar passphrase si está configurada
  ...
  if (!API_KEY || !API_SECRET) {
    log('err', 'API Key falta — configura API_KEY y API_SECRET en Railway Variables');
  ...
  try {
    ...
    if (result.retCode === 0) {
      log('ok', `✅ Orden ${data.action} ${data.symbol} — ID: ${result.result?.orderId}`);
...
async function executeTrade(side, symbol, price) {
  ...
  // Setear apalancamiento (ignora error si ya está seteado)
  try {
    await placeOrder({
